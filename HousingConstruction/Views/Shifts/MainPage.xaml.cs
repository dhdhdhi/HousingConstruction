...
namespace HousingConstruction.Views.Shifts
{
    ...
    public partial class MainPage : Page
    {
        ...
        public MainPage(NavigationContext navigationContext)
        {
            ...
            _dbContext.Shift.Load();

            DataGrid_Main.ItemsSource = _dbContext.Shift.Local.ToBindingList();
        }
        ...
        {
            var record = DataGrid_Main.SelectedItem as Shift;
            if (record == null)
            {
        ...
        private void Delete_Button_Click(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                if (MessageBox.Show("Вы уверены?", "Удаление записи.", MessageBoxButton.OKCancel) == MessageBoxResult.OK)
                {
                    try
                    {
                        _dbContext.Shift.Remove(record);
